...
genus_lacto.fun <- function(df){
  genus_lacto <- df %>%
    mutate(Genus_lacto = .$Genus, .after=Seq_ID) %>% #ifelse(grepl("Lactobacillus",.$Genus), paste(.$Genus, .$Species),
    mutate(Genus_lacto = ifelse(grepl("crispatus",.$Species), paste0(.$Genus_lacto, "crispatus/acidophilus"), 
...
```{r, echo = FALSE}
# Load metadata
dataset_names <- c("ASV_tissue_B2.csv",   # Tissue, Boston run 2 (95 samples)
                     "ASV_tissue_B1.csv", # Tissue, Boston run 1 (1 sample)
                     "ASV_CVL_V2_B1.csv", # CVL V2, Boston run 1 (27 samples)
                     "ASV_CVL_V2_B2.csv", # CVL V2, Boston run 2 (49 samples)
                     "ASV_CVL_V2_C.csv",  # CVL V2, CTMR         (62 samples)
                     "ASV_CVL_V3.csv") 
...
temp <- map(datasets_, ~.x  %>%
  ...
  {. ->> temp_ } %>% 
  aggregate.fun(., "Genus_lacto") 
)

# Extract taxonomic information to file
seq_taxa <- bind_rows(temp_) %>% select(1:11) %>% unique() 
#write.csv(seq_taxa, paste0(PATH,"results/","SeqID_to_taxa",".csv"),row.names = F )
...
datasets[["ASV_CVL_V2"]] <- as.data.frame.array(cbind( batch1[nonzeros,] , batch2_decoded_data[,samples_not_batch1] ))
...
common_microbes.fun <- function(x) {
  xy <- rownames_to_column(x, var = "Taxa")
  d <- data.frame(Taxa = common_microbes) %>%
    left_join(., xy, by="Taxa") %>%
    mutate(across(where(is.numeric), ~replace_na(.x, 0))) %>%
    column_to_rownames(var = "Taxa")
  return(d)
}
datasets <- map(datasets, ~common_microbes.fun(.x))
lapply(datasets,dim)
