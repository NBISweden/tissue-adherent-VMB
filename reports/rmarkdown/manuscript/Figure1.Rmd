---
title: "Main Figure 1.Luminal Patient Grouping"
output: 
  pdf_document:
    fig_caption: yes
    keep_tex: yes
...
```{r Figure 1., echo=FALSE, fig.asp=1, fig.height=7, fig.pos="t", fig.width=6, message=FALSE, warning=FALSE}
figlabels <- letters
temp_legend <- NULL
...
layout(matrix(c(1,1,1,1,1,1,
                ...
                5,4,7,7,8,8) , ncol= 6,byrow = T),
                               widths = c(1.9,1.4,.7,.8,.8,.7,.8),
                               heights = c(1.6,.65,1.5,.95,.9,.2))
### A
#################################
# COHORT SCHEMATIC ILLUSTRATION #
#################################
par(mar=c(0,3.3,0,1))
fig1a <- magick::image_read("/Users/vilkal/work/Brolidens_work/Projects/Gabriella_repo/supplementary_files/Scematic figure.pdf", density = 900 )
...
for(i in ASV){
  ...
  div <- tibble(shann, simp, invsimp ) %>% rename_with(., ~paste0(c("Shannon","Simpson","Inv Simpson")))
...
title(main = "Louvain", line = -2, cex.main = 1)
...
plot( g , vertex.label.cex=0.000001 , vertex.color = pal[factor(group_annotation)] , 
      ...
      edge.color=colorRampPalette(c("grey95","black"))(90) [ round( E(g)$weight / max(E(g)$weight) * 88 )+1 ] )
title(main = "UMAP", line = -2, cex.main = 1)
