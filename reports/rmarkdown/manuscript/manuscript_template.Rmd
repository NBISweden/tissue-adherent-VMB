...
```{r, echo = FALSE, eval = TRUE , fig.asp=6/10}
...
layout(matrix(c(1,1,1,1,      3,3,3,3,          4,4,4,4,
       ...
       widths = 1,heights = c(1,1,0.5,0.5,0.5))
# layout.show(29)
figlabels <- letters
...
o <- order( rowSums(temp) , decreasing = F)
sample_o <- order( as.numeric(metadata$group_M4_CVLv3)+(shann/max(shann)/2) )
temp <- temp[o,sample_o]*100
barplot( temp, las=2 , border = NA , yaxs="i", xaxs="i", col = pal[factor(rownames(temp))] )
