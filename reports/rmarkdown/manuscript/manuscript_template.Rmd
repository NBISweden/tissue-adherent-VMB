...
patient_groups <- metadata$group_M4_CVLv3
...
layout(matrix(c(1,1,1,1,      3,3,3,3,          4,4,4,4,
...
par(mar=c(0,0,0,0))
plot( g , vertex.label.cex=0.000001 , vertex.color = pal[factor(metadata$joint_clustering)] ,
...
legend("topright",title.adj = 0,
       legend = levels(factor(metadata$joint_clustering)),xjust = 0,yjust = 1,
...
shann <- vegan::diversity(t(x),index = "shannon")
shann <- t(as.matrix(shann))
rownames(shann) <- "Shannon"
barlist( data = shann,
         main = "Shannon",
         labels = "",
         genes = c("Shannon"),
...
sample_o <- order( metadata$joint_clustering )
temp <- temp[o,sample_o]*100
temp <- temp[-grep("NA",rownames(temp)),]
temp <- rbind(temp[1:15,], Other = colSums(temp[16:nrow(temp),]) )

taxa_pal <- c(RColorBrewer::brewer.pal(8,"Pastel2"),RColorBrewer::brewer.pal(8,"Pastel1"))
barplot( temp, las=2 , border = NA , yaxs="i", xaxs="i",
         col = taxa_pal[factor(rownames(temp))] )
legend(x = par("usr")[c(2)],
       y = par("usr")[c(4)],title.adj = 0,
       legend = rownames(temp),xjust = 0,yjust = 1,
       bty = "n",pch = 22,pt.cex = 2,cex = .8,xpd=T,
       pt.bg = taxa_pal)
