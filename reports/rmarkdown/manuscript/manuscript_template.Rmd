...
#   
#   plot_dots(data=module_means_per_dataset[[j]],
...
plot_dots_difference(data = module_means_per_dataset[[2]],
                     ...
                     clustering2 = as.character(group_annotation),
                     genes = rownames(module_means_per_dataset[[1]]),
                     main = "CVL3 vs tissue_V3",cex.row = .8)
# mypar(mar=c(2,2,2,6))
# plot(c(-2,10),c(-2,10),xaxs="i",yaxs="i")
# rrr <- rev(as.raster(colorRampPalette(c("blue","navy","grey95","firebrick","red"))(99)))
# rasterImage(as.raster(rrr),
#             xleft = par("usr")[2]+diff(par("usr")[1:2])/40,
#             xright = par("usr")[2]+diff(par("usr")[1:2])/20,
#             ybottom = par("usr")[4]-diff(par("usr")[1:2])/4,
#             ytop = par("usr")[4], 
#             interpolate = FALSE )
# text( c(par("usr")[2]+diff(par("usr")[1:2])/20,
#         par("usr")[2]+diff(par("usr")[1:2])/20) ,
#       c(par("usr")[4]-diff(par("usr")[3:4])/4,
#         par("usr")[4]),labels = c("min","max"),pos=4 )
# 
# 
# text( c(par("usr")[2]+diff(par("usr")[1:2])/20,
#         par("usr")[2]+diff(par("usr")[1:2])/20,
#         par("usr")[2]+diff(par("usr")[1:2])/20) ,
#       c(par("usr")[4]-diff(par("usr")[1:2])/4,
#         par("usr")[4]-diff(par("usr")[1:2])/8,
#         par("usr")[4]),labels = c("-1","0","+1"),pos=4 )
# text( c(par("usr")[2]+diff(par("usr")[1:2])/40) ,xpd=T,
#       c(par("usr")[4]+diff(par("usr")[3:4])/40),labels = c("Z-score"),adj=0 )
...
mypar()
x <- NESse_list[[1]] - rowMeans( NESse_list[[1]] )
x [ pvalue_list[[1]] < 0.05 ] <- 0
