...
#         pch=16,cex=.5,xpd=T)
  points( (1:ncol(temp))*1.2 -.5, rep(par("usr")[3],ncol(temp))-2,
        col= c("grey","red")[(!is.na(metadata$RepeatedPCR))+1][order( group_annotation )],
        pch=15,cex=.8,xpd=T)
...
points( seq(0,1,length.out = ncol(top_picrust_data)),rep(par("usr")[3],ncol(top_picrust_data))-.005,
        col= c("grey","red")[(!is.na(metadata$RepeatedPCR))+1][order(group_annotation[sample_use])],
        pch=16,xpd=T)
...
picrust_KO_seq <- rowsum(picrust_KO_seq , picrust_taxa[ match( rownames(picrust_KO_seq) , as.character(picrust_taxa$Seq_ID)  ),"final_name"])
...
metadata <- metadata %>%
  #select(Contraception, sexwork_months) %>%
...
#design <- model.matrix(~  group_annotation, data=metadata[sample_use,])
#design <- model.matrix(~0 + group_annotation+Contraception+HIVstatus+age+sexwork_months, 
                       #data=metadata[sample_use,])
design <- model.matrix(~group_annotation+HIVstatus+Contraception+age+sexwork_months, 
                       data=metadata[sample_use,])
colnames(design) <- sub("group_annotation","",colnames(design))
...
# without intercept:
n <- combn(sort(unique(group_annotation)),2) 
...
# with intercept:
n <- combn(sort(unique(group_annotation))[-1],2) 
g <- map_chr(seq_along(1:ncol(n)), ~paste(n[1,.x], n[2,.x], sep = "-"))
g <- c(sort(unique(group_annotation))[-1], g) 
...
# save TopTables 
library(openxlsx)
l_top <- list(ModifiedM4 = top)
imap(l_top, ~write.xlsx(.x, file=paste0("../../../results/","TopTable",.y,".xlsx"))) # Excel


## individual GO comparison ##
source("../enrichment_function.R")
...
## GO enrichment
enrichment_groups <- map_depth(t, 1, ~enrich_list(rownames(.x), gmt_list_GO))
enrichment_groups_split <- map_depth(t_split, 2, ~enrich_list(rownames(.x), gmt_list_GO)) %>%
  map(., ~bind_rows(.x, .id = "regulation"))
 
## save files
l_enrich <- list(ModifiedM4_split = enrichment_groups_split, ModifiedM4 = enrichment_groups)
imap(l_enrich, ~write.xlsx(.x, file=paste0("../../../results/","GO_enrichment_",.y,".xlsx"))) # Excel
...
image( t(top_TRX_counts[h$order,order(group_annotation[sample_use])][nrow(top_TRX_counts):1,]),
       col = colorRampPalette(c("navy","navy","grey95","firebrick4","firebrick4") )(99),
       breaks = seq(-5,5,length.out = 100),axes=F , xlab="patient groups")
points( seq(0,1,length.out = length(sample_use)),rep(par("usr")[3] - .02,length(sample_use)),
        col=pal[group_annotation[sample_use][ order(group_annotation[sample_use])]],
        pch=16,xpd=T)

points( seq(0,1,length.out = ncol(top_TRX_counts)),rep(par("usr")[3],ncol(top_TRX_counts))-.005,
        col= c("grey","red")[(!is.na(metadata[sample_use,"RepeatedPCR"]))+1][order(group_annotation[sample_use])],
        pch=16,xpd=T)
points( seq(0,1,length.out = ncol(top_TRX_counts)),rep(par("usr")[3],ncol(top_TRX_counts))-.01,
        col= c("grey","red")[as.numeric(metadata[sample_use,"HIVstatus"])][order(group_annotation[sample_use])],
        pch=16,xpd=T)
...
for(j in names(module_means_per_dataset)){
  ...
  plot(UMAP_picrust,bg=pal[group_annotation],main=gsub("ASV_|_norm.*","",j),
     pch=21,xlab="UMAP1",ylab="UMAP2",frame=F,axes=F)
  add_letter(figlabels[1]); figlabels <- figlabels[-1]
...
for(j in names(module_means_per_dataset)){
  ...
  plot(UMAP_TRX,bg=pal[group_annotation[sample_use]],main=gsub("ASV_|_norm.*","",j),
     pch=21,xlab="UMAP1",ylab="UMAP2",frame=F,axes=F)
  add_letter(figlabels[1]); figlabels <- figlabels[-1]
...
plot(l,
       col=pal[metadata$group_annotation],
       main=i,xpd=T,
       pch=16,xlab="",ylab="",frame=F,axes=F,font.main=1,cex.main=1)
empty_plot()
legend("topleft",title.adj = 0,
       legend = levels(metadata$group_annotation),xjust = 0,yjust = 1,
...
plot(UMAP_TRX,
       col=pal[metadata[common,"group_annotation"]],
       main="",xpd=T,
       pch=16,xlab="",ylab="",frame=F,axes=F,font.main=1,cex.main=1)
empty_plot()
legend("topleft",title.adj = 0,
       legend = levels(metadata[common,"group_annotation"]),xjust = 0,yjust = 1,
       bty = "n",pch = 16,col = pal,pt.cex = 1,xpd=T)
...
```{r Print rare curves, fig.width=6, fig.height=10, fig.align="center"}
