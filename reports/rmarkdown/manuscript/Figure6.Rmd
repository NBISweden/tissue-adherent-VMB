...
plot_heatmap.fun <- function(top_PROT_mfi, gr, scale = FALSE, layout = "none"){
  ...
  if(layout == "single") {
    ...
    image( t(top_PROT_mfi[h$order,pull(df, "ID")][nrow(top_PROT_mfi):1,]),
           ...
           breaks = breaks,
           axes=F)
    mtext( "Differentialy Expressed Proteins", side=3, las = 1, cex = .7, line =1.5,xpd=T)
...
rect(7, -0.5, 5.5, par("usr")[2], density = NULL, angle = 45, xpd = TRUE,
     col = "White", border = "White", lty = par("lty"), lwd = par("lwd")
     )
...
par( mar=c(24.4,6,1.5,0)) #b,l,t,r
...
rect(6, -0.5, 4.5, par("usr")[2], density = NULL, angle = 45, xpd = TRUE,
     col = "White", border = "White", lty = par("lty"), lwd = par("lwd")
     )
add_letter("d")
