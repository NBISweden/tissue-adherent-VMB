...
# combinations
sheets <- map(DEG_files, ~excel_sheets(paste0(out, .x))[-c(1:2)] ) %>%
  map(., ~set_names(.x))
...
save_pdf.fun <- function(i, gr_name, comp){
  temp_i_name <- grep(i,list.files(paste0(out),full.names=F),value=T)
  p <- map(comp, ~read_xlsx(paste0(out, temp_i_name), sheet = .x)) 
  ...
  pdf(file = paste0(out, "Plots/", i, "_network_",".pdf", sep=""))
    p <- imap(obj_igraph,
         ~with(.x, 
...
# if(isFALSE(dir.exists(paste0(out,"Plots")))){dir.create(paste0("../",result.dir),recursive = TRUE)}
