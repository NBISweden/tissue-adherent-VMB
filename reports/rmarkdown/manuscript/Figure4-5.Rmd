...
## GO enrichment 
pairwise_GO <- map_depth(t_split, 3, ~enrich_list(.x$Genes, gmt_list_GO)) %>%
  map_depth(., 2, ~bind_rows(.x, .id = "regulation")) 
## KEGG enrichment
pairwise_KEGG <- map_depth(t_split, 3, ~enrich_list(.x$Genes, gmt_list_KEGG)) %>%
  map_depth(., 2, ~bind_rows(.x, .id = "regulation")) 
...
write.xlsx(pages, file=paste0(out,DEG_files[1]))

pages <- list(list(Across=dge_T$top), pairwise=pairwise_dge_T$top) %>% flatten()
write.xlsx(pages, file=paste0(out,DEG_files[2]))
```
...
write.xlsx(df_across[[1]]$res, file=paste0(out,"Suppl.Tbl.05 Enrichment_Across_Luminal",".xlsx"))
write.xlsx(df_across[[2]]$res, file=paste0(out,"Suppl.Tbl.09 Enrichment_Across_Tissue",".xlsx"))
...
write.xlsx(df_pairwise[[1]]$value, file=paste0(out,"Suppl.Tbl.06 Enrichment_Pairwise_Luminal",".xlsx"))
write.xlsx(df_pairwise[[2]]$value, file=paste0(out,"Suppl.Tbl.10 Enrichment_Pairwise_Tissue",".xlsx"))
```

```{r TF PPI Analysis function, include=FALSE}
TF_PPI_analysis.fun <- function(out, filename, gr_name, outfile) {
  # Get DEG's files and filter p-value < 0.01
  sheets <- excel_sheets(paste0(out, filename)) %>% set_names(.) %>% .[-c(1:2)]
  detables <- map(sheets, ~read_xlsx(paste0(out, filename), sheet = .x, col_names = TRUE)) %>%
    ...
    write.xlsx(save_l, file=paste0(out, outfile), row.names=FALSE)
  }
```

```{r TF PPI Analysis, eval=FALSE, include=FALSE}
TF_PPI_analysis.fun(out, DEG_files[1], gr_name[1], enrichR_files[1]) # writes Suppl.Tbl.07
TF_PPI_analysis.fun(out, DEG_files[2], gr_name[2], enrichR_files[2]) # writes Suppl.Tbl.11
```
...
plot_heatmap.fun <- function(enrich, db, gr, name, N=3, Layout){
  ...
  mtext("           Differentialy Expressed Genes", side=3, las = 1, cex = .6, line =1.5,xpd=T, font = 2)
  ...
  axis(1, at = c(start, end), label = F, pos =1.027 , xpd=T, col="white",col.ticks="black")
  map2(start, end, ~lines(x=c(.y, .x), y= c(1.027, 1.027), xpd=T, cex=.8))
  # text labels
  ...
  text(par("usr")[1],par("usr")[3]-.028,labels = "    HIV",cex=.6, xpd=T)
...
arrange_plots.fun <- function(degs, group, gr_name, comparison, enrichment_db, file, Layout){
  ...
  for( i in enrichment_db ){
    ...
    f_DOWN <- read_xlsx(paste0(out, file), sheet = x) %>% filter(Regulation == "down")
    f_UP <- read_xlsx(paste0(out, file), sheet = x) %>% filter(Regulation == "up")
    ...
    barplot( c(rep(NA, each=20-length(comb)), comb),  
             xlim=c(-1.1,1.1),
             # xlim=c(min(c(comb,-2/abs(max(abs(DD))) ))*1.1, 
             #                          max(c(comb,2/abs(max(abs(DD)))) )*1.1), 
                        cex.names = .9, #mar=c(1,25,2,2), #main=paste(x, i, sep="\n"),
             names.arg = c(rep("", each=20-length(comb)),names_DD, names_UU ), 
             horiz = T,las= 1, border=NA,xaxt='n',line=-.2, #xlab="-log10 p-value",
...
sheets <- excel_sheets(paste0(out, DEG_files[1])) %>% set_names(.) %>% .[-c(1:2)]
arrange_plots.fun(enrich_L, "Luminal_gr", "Luminal", sheets, "TF_PPI_enrichment", enrichR_files[1], Layout = l)
```

```{r Figure 5., message=FALSE, warning=FALSE, echo=FALSE, fig.height=6}
# comparison <- sheets
...
sheets <- excel_sheets(paste0(out, DEG_files[2])) %>% set_names(.) %>% .[-c(1:2)]
...
write.xlsx(pages, file=paste0(out,"Suppl.Tbl.12 L2T2_L2T3_DEGs_and_enrichment.xlsx")) 
```




```{r UMAP, eval=FALSE, fig.height=2.5, fig.width=3, include=FALSE}
...
## igraph network plot
```{r igraph network, echo=FALSE, warning=FALSE} 
# 1. Change the groups of interest by assigning condition_use
# 2. Change the number of genes to be included in the network by changing n       # , results="hold"
...
i_graph <- function(temp_i, n, square, cricle){
  ...
  names(temp) <- temp_i$Term[1:length(temp)]
...
# condition_use <- g[1]
# g <-  g[1]
# temp_i_names <- paste0(strsplit(i, "enrichment"),condition_use,"_UP_", gr_name,".csv") %>% set_names()

# Save all combination plots in one file
save_pdf.fun <- function(i){
  
  temp_i_names <- grep(gr_name,list.files(paste0(out,i,"/",gr_name),full.names=F),value=T) %>%
    set_names()
  temp_i <- imap(temp_i_names, ~read.csv2(paste0(out,i,"/",gr_name,"/",.x)))
  obj_igraph <- imap(temp_i, ~i_graph(.x, n, square, cricle))
  pattern <-paste(g , collapse="|")
  
  par(mar = c(0, 0, .7, 0))  #bottom, left, top, right
  p <- imap(obj_igraph,
       ~with(.x, 
               plot.igraph(g, label=V(g), 
               main = paste0(str_extract(.y,pattern)," ",str_extract(.y, "UP|DOWN")," regulated genes"),
  ...
  #text(0,1.15, paste0(str_extract(.y,pattern)," ",str_extract(.y, "UP|DOWN")," regulated genes"), cex=1.2, font=2)
  dev.copy2pdf(file = paste0(out, "Plots/", gr_name, "_network_", i,".pdf", sep=""))
}
