...
plot_heatmap.fun <- function(top_PROT_mfi, gr, scale = FALSE, layout = "none"){
  ...
  if(layout == "multi") {
    layout(matrix(c(1,2,5,6,
           ...
           widths = c(.9,2,.9,2), heights = c(1,1,.01) ) }
  if(layout == "single") {
    layout(matrix(c(1,2),
    ...
    ## Dendogram
    par(mgp=c(0,0.2,0), mar=c(4,.6,1,.2)) #(bottom, left, top, right) 
    ...
    ## Heatmap
    par(mar=c(4,.2,1,2.5)) #(bottom, left, top, right) 
    ...
    mtext( c("patient groups"), side=1, las = 1, cex = .7, line =1,xpd=T)
...
```{r Save stats to excel, include=FALSE}
# save TopTables pairwise
l_top_p <- list(Luminal = pairwise_dpe_L$top, Tissue = pairwise_dge_T$top)
imap(l_top_p, ~write.xlsx(.x,file=paste0("../../../results/DE/","dpe_pairwise_",.y,".xlsx"))) # Excel

# save TopTables across + cytokines
l_top_a <- list(Luminal = list(Across = dpe_L$top), Tissue = list(Across = dpe_T$top))
l <- list(l_top_a, stat, l_top_p)
names <- c("Luminal", "Tissue") %>% set_names()
p_list <- imap(names, ~flatten(map_depth(l, 1, .x))) 

imap(p_list, ~write.xlsx(.x,file=paste0("../../../results/DE/","Proteins_",.y,".xlsx"))) # Excel

```

```{r Proteins plot, echo = FALSE, eval = TRUE, fig.height=10, fig.width=7}
figlabels <- letters
