...
samples_to_use <- colnames(counts)[(colSums(counts == 0)<3)]
...
all_microbiome <- all_microbiome[ rowSums(all_microbiome>0)>=2 ,  ]
dim(all_microbiome)
...
cors <- cor(all_microbiome[is_top_var,], method = "pearson")
adj <- (1-cors)/2
...
#Graph construction  
k <- 15 # k for KNN
P <- 0.1# prunning threshold of SNN
...
# k=10
# P=0.1
snn <- dbscan::sNN(x = adj, k = k, jp = T, approx = TRUE)
snnm <- matrix(0, nrow(snn$id), nrow(snn$id))
snnj <- matrix(0, nrow(snn$id), nrow(snn$id))

# for(idx in 1:nrow(snn$id) ){  snnm[idx,snn$id[idx,]] <- snn$dist[idx,] }
for(idx in 1:nrow(snn$id) ){  snnm[idx,snn$id[idx,]] <- snn$shared[idx,]/k }

for(idx in 1:nrow(snn$id) ){  snnj[idx,snn$id[idx,]] <- snn$shared[idx,] }

snnm[ snnj <= ceiling(k*P) ] <- 0
...
# image(J[,nrow(nn):1],col = colorRampPalette(c("grey95","black"))(90),axes=F )
# set.seed(1)
# g <- graph_from_adjacency_matrix(J,mode = "undirected",diag = F,weighted = T)
# plot( g , vertex.label.cex=0.000001 , vertex.color = "grey" , 
...
metadata$joint_clustering <- cl$membership
set.seed(1)
...
plot( g , vertex.label.cex=0.000001 , vertex.color = pal[factor(metadata$group_M4_CVLv3)] , 
      edge.width=  ( E(g)$weight / max(E(g)$weight)) ,layout=l,
      vertex.size=10,main="FR embedding",
      edge.color=colorRampPalette(c("grey95","black"))(90) [ round( E(g)$weight / max(E(g)$weight) * 88 )+1 ] )
plot( g , vertex.label.cex=0.000001 , vertex.color = pal[factor(metadata$group_M4_CVLv3)] , 
...
empty_plot()
legend("topleft",title = "Patient groups\ngroup_M4_CVLv3",
       legend = levels(factor(metadata$group_M4_CVLv3)),
       bty = "n",pch = 21,pt.bg = pal,pt.cex = 1,xpd=T)
...
```{r, eval=F, echo = FALSE,fig.width=10,fig.height=4}
...
datasets <- factor(c( rep("tissue_v3",ncol(datasets_all_samples[["ASV_tissue_V3_normalized_batch_corrected"]])) ,
                      rep("CVL_v3",ncol(datasets_all_samples[["ASV_CVL_V3_normalized_batch_corrected"]])) ,
                      rep("CVL_v2",ncol(datasets_all_samples[["ASV_CVL_V2_normalized_NOT_batch_corrected"]])) ))

groups <- factor( rep(metadata$joint_clustering,3) )
age <- rep(metadata$age,3)
work_time <- rep(metadata$sexwork_months,3)

datasets <- datasets[ colSums(all_microbiome)>0 ]
groups <- groups[ colSums(all_microbiome)>0 ]
age <- age[ colSums(all_microbiome)>0 ]
all_microbiome <- all_microbiome[,colSums(all_microbiome)>0]

df <- data.frame(groups,datasets,age)

library(edgeR)
design <- model.matrix( ~ datasets * groups + age , data=df )
rownames(design) <- paste0(datasets,"_",colnames(all_microbiome))
colnames(all_microbiome) <- rownames(design)


y <- DGEList(counts = round(((2^all_microbiome)-1)*1000) , remove.zeros = T)
...
#age
lrt <- glmLRT(fit,coef = 4:7)
top <- topTags(lrt,adjust.method = "BH",n = "all",sort.by = "p.value")[[1]]



#age
lrt <- glmLRT(fit,coef = 9:12)
top <- topTags(lrt,adjust.method = "BH",n = "all",sort.by = "p.value")[[1]]



#age
lrt <- glmLRT(fit,coef = 13:16)
top <- topTags(lrt,adjust.method = "BH",n = "all",sort.by = "p.value")[[1]]
...
```{r, eval=F,echo = FALSE,fig.width=10,fig.height=9}
all_microbiome <- cbind(datasets_all_samples[["ASV_tissue_V3_normalized_batch_corrected"]],
                        datasets_all_samples[["ASV_CVL_V3_normalized_batch_corrected"]],
                        datasets_all_samples[["ASV_CVL_V2_normalized_NOT_batch_corrected"]])
...
metadata_parameters <- c("Antibiotics","Antibiotics_v2","Antibiotics_v3",
                         ...
                         "vaginal_douching_v2","vaginal_douching_v3","joint_clustering")
...
clean_Ftest[clean_Ftest > 5.5] <- 5.5
# clean_Ftest[ clean_Ftest > 0.05 ] <- 1

write.csv2(clean_Ftest , "../../results/metadata_association_results.csv",row.names = T)
...
cors <- cor(t(all_microbiome[rowSums(all_microbiome > 0)>=5,]))

#Graph construction 
k_nearest_neighbours <- 10 # k for KNN
...
#     
#     barlist(data=module_means_per_dataset[[j]],
#             genes=rownames(module_means_per_dataset[[j]]),
#             labels = paste0("bac.com.",rownames(module_means_per_dataset[[j]])),
#             clustering=temp_group, main=paste0(i,"\n", sub("_norm.*","",j)),srt=45)
#   }
# }
  
for( i in c("group_M4_CVLv3","group_M6_CVLv3","group_M9_CVLv3","joint_clustering")){
  for( j in c("ASV_tissue_V3_normalized_batch_corrected",
          "ASV_CVL_V3_normalized_batch_corrected",
          "ASV_CVL_V2_normalized_NOT_batch_corrected")){
...
for(i in c("ASV_tissue_V3_normalized_batch_corrected",
  ...
  invisible(parallel::clusterEvalQ(cl, {c("gmt","cors");library(fgsea);library(stats);library(base)}))
  enrichments <- parallel::parLapply(cl,bacteria_use,gmt=gmt,cors=cors,function(x,gmt,cors){
    res <- fgseaMultilevel(pathways = gmt,stats = sort(cors[,x],decreasing = T))
    return(res)
  })
  parallel::stopCluster(cl)
  ...
  NESes[is.na(NESes)] <- 0
  NESes[ pvalues < -log10(0.05) ] <- 0
  # NESes <- NESes[rowSums(NESes!=0) >= 2 , colSums(NESes!=0) >= 2]
  ...
  # image( t(NESes[nrow(NESes):1,]),col=colorRampPalette(c("navy","grey95","firebrick"))(91),breaks=seq(-3,3,length.out = 92),
  #        axes=F,border=NA,main=i,xlab="",ylab="",line=.4,cex.main=1,font.main=1)
  # text(  par("usr")[c(4)] , seq(1,0,length.out = nrow(NESes)),
  #       labels = rownames(NESes), srt = 0, adj = c(0,.5), xpd = TRUE, cex=1)
  # text( seq(0,1,length.out = ncol(NESes)) , par("usr")[c(1)],
  #       labels = colnames(NESes), srt = 90, adj = c(1,.5), xpd = TRUE, cex=.4)
...
for( i in names(NESse_list)){
  ...
  # add_letter(myletter); myletter <- myletter[-1]
  text(   seq(1,0,length.out = nrow(x)) , par("usr")[c(3)] ,
...
TRX <- TRX [ top_vars_TRX , non_zeros ]
...
plot_dots(data=pathway_means,main="patient groups",cex.main=1,font.main=1,
        ...
        clustering=metadata$joint_clustering[non_zeros],
...
common_pathways <- unique( unlist( lapply( NESse_list , function(x) colnames(x)[ colSums(x!=0)>=2 ] ) ) )
