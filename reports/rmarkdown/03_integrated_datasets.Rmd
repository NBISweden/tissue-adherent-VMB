...
metadata_parameters <- c("Antibiotics","Antibiotics_v2","Antibiotics_v3",
                         ...
                         "group_M9_CVLv3","group_M6_CVLv3","group_M4_CVLv3")
...
#Graph construction  
k <- 10 # k for KNN
P <- 0.3 # prunning threshold of SNN
...
points( (0:(length(df$louvain_tissue_V3)-1) )/(length(df$louvain_tissue_V3)-1),
        rep(par("usr")[c(4)]+.01*diff(par("usr")[c(3,4)]),
            length(df$louvain_tissue_V3)), pch=16,
        col=pal[factor(df$louvain_tissue_V3)][sample_order] ,xpd=T)
text( rep(par("usr")[c(1)]-.01*diff(par("usr")[c(1,2)]), length(sort(unique(cl$membership))) ),
         (0:(length( sort(unique(cl$membership)) )-1) )/(length(sort(unique(cl$membership)))-1),
         labels= rev(sort(unique(cl$membership)) )    )
...
**Figure 5.** Comparisson of microbiome datasets, showing only the distribution of different bacterial communities (1-8). Samples are ordered by the CVL3 groupings. The colors represent their respective bacterial groupings for each dataset. 
...
for(i in c("ASV_tissue_V3_normalized_batch_corrected",
  ...
  TRX <- TRX [ , samples_TRX_and_CVL3 ]
  TRX <- TRX [ rowSums(TRX>0)>= 2 , ]
  top_vars_TRX <- names(sort(apply(TRX,1,var),decreasing = T)[1:2000] )
  # plot(apply(TRX,1,mean),apply(TRX,1,var),col=ifelse(rownames(TRX) %in% top_vars_TRX,"red","grey20"),cex=.2)
  TRX <- TRX[ top_vars_TRX , ]
  dim(TRX)
