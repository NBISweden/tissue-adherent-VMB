...
#color palettes
pal <- c(RColorBrewer::brewer.pal(9,"Set1"),RColorBrewer::brewer.pal(8,"Set2")) #color pallete for plots
...
#Graph construction  
k <- 10 # k for KNN
P <- 0.4 # prunning threshold of SNN
...
# a <- read.csv(paste0(PATH,"/ASV_tissue_B2.csv"))
# b <- read.csv(paste0(PATH,"/ASV_tissue_B1.csv"))
# rownames(a) <- paste0()
# temp <- as.matrix(a[,c((ncol(a)-6):ncol(a))])
# temp[is.na(temp)] <- "NA"
# taxa <- apply( temp ,1,function(x) paste(x,collapse ="_"))
# taxa <- sub("_NA.*","_other",taxa)
...
# datasets <- datasets[2] #in case you wanna test in 1 dataset
...
microbiome <- as.matrix(datasets[[3]])
...
zscore_bac <- apply(microbiome,2,function(i){scale(i,T,T)})
zscore_bac[ zscore_bac > 6 ] <- 6
zscore_bac[ zscore_bac < -6 ] <- -6
rownames(zscore_bac) <- rownames(microbiome); colnames(zscore_bac) <- colnames(microbiome)

#Compute sample-wise correlations
cors <- cor(t(microbiome),method = "pearson")
...
cl_bac <- igraph::cluster_louvain(g)

plot( g , vertex.label.cex=0.000001  , 
      edge.width=  ( E(g)$weight / max(E(g)$weight)) ,layout=l,
      vertex.color= hue_pal()(length(unique(cl_bac$membership)))[factor(cl_bac$membership)],
      vertex.size=10,
      edge.color=colorRampPalette(c("grey95","black"))(90) [ round( E(g)$weight / max(E(g)$weight) * 88 )+1 ] )

legend(x = par("usr")[c(2)]+.2*diff(par("usr")[c(1,2)]),
       y = par("usr")[c(4)],
       legend = levels(factor(cl_bac$membership)),pt.bg=hue_pal()(length(unique(cl_bac$membership))),
       bty = "n",pch = 21,pt.cex = 1,xpd=T)
title("Louvain clusters",cex.main=1,font.main=1)


o_bacs <- order(cl_bac$membership)
image( t(zscore[o_bacs,o][nrow(zscore):1,]),col=heat_pal,axes=F,border=NA,
      main="bacteria",xlab="",ylab="samples",line=.4,cex.main=1,font.main=1)

text(  par("usr")[c(4)]+.01*diff(par("usr")[c(3,4)]) , seq(1,0,length.out = nrow(zscore)),
      labels = rownames(zscore), srt = 0, adj = c(0,.5), xpd = TRUE, cex=.7)
points( rep(par("usr")[c(1)]-.01*diff(par("usr")[c(1,2)]),length(cl_bac$membership)),
        (0:(length(cl_bac$membership)-1) )/(length(cl_bac$membership)-1), pch=16,
    rev((0:(length(cl$membership)-1) )/(length(cl$membership)-1)),
    col=pal[factor(cl$membership)][o_bacs] ,xpd=T)
...
points( (0:(length(cl$membership)-1) )/(length(cl$membership)-1),
        rep(par("usr")[c(3)]-.01*diff(par("usr")[c(3,4)]),length(cl$membership)), pch=16,
        col=pal[factor(cl$membership)][o] ,xpd=T)
points( rep(par("usr")[c(1)]-.01*diff(par("usr")[c(1,2)]),length(cl_bac$membership)), pch=16,
    rev((0:(length(cl_bac$membership)-1) )/(length(cl_bac$membership)-1)),
    col=pal[factor(cl_bac$membership)][o_bacs] ,xpd=T)
...
```{r, eval=F, echo = FALSE, fig.width  = 10, fig.height=6}
# Load the transcriptome datset
trx <- read.csv(paste0(PATH,"~/Downloads/TRX_rawcounts.csv"),row.names = 1)
...
# Identify highly variable genes in the transcriptomic dataset
vars <- log2(apply(trx,1,var)+2)
means <- apply(trx,1,mean)
mypar(1,2)
top_variable <- names( sort(vars,decreasing = T) ) [1:3000]
plot( means , vars , pch=16, las=1,xlab="logMeanExpression",ylab="variance",
      cex=ifelse( rownames(trx) %in% top_variable , .2,.1),
      col= ifelse( rownames(trx) %in% top_variable , "red","black"))

# Identify common samples between the transcriptome and the mirobiome dataset
common_samples <- colnames(microbiome)[colnames(microbiome) %in% colnames(trx)]
trx <- trx[,as.character(common_samples) ]
microbiome2 <- microbiome[,as.character(common_samples)]


# Combine datased and set all to same scale using Z-score transformation
mypar(1,3)
merged <- rbind(trx[top_variable,],microbiome2)
zscore_merged <- t(apply(merged,1,function(i){scale(i,T,T)}))
zscore_merged[ zscore_merged > 5 ] <- 5
zscore_merged[ zscore_merged < -5 ] <- -5
rownames(zscore_merged) <- rownames(zscore_merged)
colnames(zscore_merged) <- colnames(zscore_merged)
hist(zscore_merged,breaks = 100,main="all")
hist(zscore_merged[top_variable,],breaks = 100,main="trx")
hist(zscore_merged[rownames(microbiome2),],breaks = 100,main="microbiome")


PC <- prcomp(zscore_merged,center = F,scale. = F)
plot(PC$sdev,pch=21,bg="grey70")
abline(v=50.5,col="red",xpd=F,lty=2)

knn <- RANN::nn2( query = PC$x[rownames(microbiome2),1:50] , 
                data = PC$x[top_variable,1:50] ,k = 50)
# knn <- RANN::nn2(t(zscore),k = k)
nn <- matrix(0, length(rownames(microbiome2)), length(top_variable))
for(idx in 1:nrow(knn$nn.idx) ){  nn[idx,knn$nn.idx[idx,]] <- 1 }
nn <- nn[ rowSums(nn)>0 , colSums(nn)>0 ]

mypar(1,2,mar=c(0,0,2,10))

g <- graph_from_adjacency_matrix(nn, mode = "undirected",diag = F,weighted = T)
set.seed(1)
# l <- layout_with_graphopt(g,niter = 1000)
l <- layout_with_fr(g,niter=500,start.temp=100)
...
```{r, eval=F,echo = FALSE, fig.width  = 10, fig.height=11}
